...
flowchart LR
    subgraph Ingestion
        J[Jira tickets] --> P[Preprocess + chunk]
        D[HTML docs] --> P
        P --> E[Embed]
        E --> Q[(Qdrant + doc store)]
    end
    U[User query] --> R[Retrieve top-k]
    Q --> R
    R --> RR[Source-aware reranking]
    RR --> PR[Prompt build]
    PR --> L[LLM generation]
    L --> A[Grounded answer]
    A --> EV[RAGAS evaluation + MLflow]
```

## Results

Polaris was evaluated on a **hand-annotated benchmark of 100 HPC-support queries** (70 dev / 30 test), each labelled by the evidence it needs (`docs` / `tickets` / `both`), documentation scope, validity-sensitivity, and attachment-dependence. Answers are scored with **RAGAS** (`context_recall`, `faithfulness`, `factual_correctness`), each condition repeated 3× for stable means with bootstrap confidence intervals.

The evaluation is a multi-stage protocol covering **generator selection** (Llama-3.3-70B vs Mistral-Large vs Mixtral-8x22B), **chunking studies** (ticket and document chunk sizes/overlaps), **source ablation** (docs-only vs tickets-only vs combined), and **validity-aware reranking**.

On the held-out benchmark, the combined system reached approximately **context recall 0.66, faithfulness 0.50, factual correctness 0.32** (mean of 3 runs) — modest absolute scores that reflect a deliberately hard, validity-sensitive support task where many answers depend on current systems, versions, and policy. The contribution is the **evaluation methodology and source-aware reranking**, not a single headline number.

Full per-stage results tables (leaderboards, condition aggregates, bootstrap CIs) are in [`artifacts/experiments/`](artifacts/experiments/), and the methodology is documented in [`METHODOLOGY.md`](METHODOLOGY.md).

> The numbers above come from the original evaluation on the private real-ticket corpus. The synthetic corpus shipped here lets you re-run the *pipeline and harness*; its scores will differ.

## User Interface

The React SPA provides assistant, evaluation, and system views. The screenshots below use the **synthetic demo data** shipped in this repo.

**Assistant** — the landing view surfaces example scenarios; ask about tickets, docs, services, or queue issues:

![Polaris assistant](docs/screenshots/ui-assistant.png)

**System overview** — the retrieval-to-answer pipeline at a glance:

![Polaris architecture](docs/screenshots/ui-system.png)
...
Polaris was developed against a private corpus of **real** HPC helpdesk tickets, which often contain personal data, credentials, internal hostnames, and other sensitive content. **None of that real data is included in this repository.**

This public version ships only a **fully synthetic** corpus and evaluation set under `data/synthetic/`, generated by `scripts/generate_synthetic_corpus.py` and `scripts/generate_synthetic_eval.py`. The tickets are fabricated — no real ticket, person, email, or identifier is represented. The synthetic data mirrors the real corpus's *category distribution* and schema so the pipeline is realistic and runnable, but contains no real content.

If you point Polaris at your own ticket source, treat all data as confidential, keep secrets in `.env` (never commit them), and avoid committing PII or generated stores (see `.gitignore`).
