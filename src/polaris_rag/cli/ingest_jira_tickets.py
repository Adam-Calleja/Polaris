...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Ingest tickets from a local JSONL file instead of fetching from Jira. "
            "Each line must be a Jira-shaped ticket object. Use this to ingest the "
            "synthetic demo corpus offline, e.g. data/synthetic/tickets.jsonl. "
            "When set, --start-date/--end-date/--limit are ignored."
        ),
    )
    parser.add_argument(
        "--exclude-keys-file",
...
def _iter_ticket_batches_from_file(path: str, batch_size: int | None):
    """Yield batches of raw ticket dicts from a JSONL file (offline ingestion).

    Each line must be a single Jira-shaped ticket object (the same schema the
    Jira API returns). This backs the ``--tickets-file`` option so the pipeline
    can ingest a local corpus, such as the synthetic demo data, without Jira
    access. Yields lists matching ``iter_support_ticket_batches``.
    """
...
def main() -> None:
    ...
    if args.tickets_file:
        print(f"Loading tickets from file: {args.tickets_file}")
        ticket_batches = _iter_ticket_batches_from_file(args.tickets_file, fetch_batch_size)
    else:
        ticket_batches = iter_support_ticket_batches(
    ...
    for batch_index, fetched_tickets in enumerate(ticket_batches, start=1):
        total_fetched += len(fetched_tickets)
        source_label = "file" if args.tickets_file else "Jira"
        print(f"Fetched {source_label} batch {batch_index}: {len(fetched_tickets)} tickets.")
