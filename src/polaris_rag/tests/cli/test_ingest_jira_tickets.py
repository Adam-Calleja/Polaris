...
def test_main_markdown_chunking_path_persists_markdown_tickets(monkeypatch):
    ...
    markdown_documents = [
        MarkdownDocument(
            ...
            text="# Ticket DEMO-1",
            metadata={"ticket_key": "DEMO-1", "summary": "Summary"},
        )
    ]
    chunks = [
        DocumentChunk(
            id="DEMO-1::chunk::0000",
            parent_id="DEMO-1",
            prev_id=None,
            next_id=None,
            text="# Ticket DEMO-1",
    ...
    monkeypatch.setattr(
        ...
        lambda start_date, end_date, limit, cfg, exclude_keys, batch_size: iter(
            [[{"key": "DEMO-1", "fields": {"summary": "Summary"}}]]
        ),
    )
    monkeypatch.setattr(
        ingest_jira_tickets,
    ...
    assert deleted_ids == ["DEMO-1"]
    assert deleted_from_docstore == ["DEMO-1"]
...
def test_main_processes_jira_tickets_in_fetch_batches(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda start_date, end_date, limit, cfg, exclude_keys, batch_size: iter(
            [
                [
                    {"key": "DEMO-1", "fields": {"summary": "Summary 1"}},
                    {"key": "DEMO-2", "fields": {"summary": "Summary 2"}},
                ],
                [
                    {"key": "DEMO-3", "fields": {"summary": "Summary 3"}},
                ],
            ]
        ),
    )
    ...
    assert inserted_batches == [["DEMO-1", "DEMO-2"], ["DEMO-3"]]
    assert deleted_ids == ["DEMO-1", "DEMO-2", "DEMO-3"]
    assert deleted_from_docstore == ["DEMO-1", "DEMO-2", "DEMO-3"]
    assert persisted_docs == ["DEMO-1", "DEMO-2", "DEMO-3"]
...
def test_iter_ticket_batches_from_file_batches_and_skips_blanks(tmp_path):
    import json

    path = tmp_path / "tickets.jsonl"
    tickets = [{"key": f"DEMO-{i}", "fields": {"summary": f"s{i}"}} for i in range(5)]
    lines = [json.dumps(t) for t in tickets]
    # Insert a blank line that must be skipped.
    path.write_text("\n".join([lines[0], lines[1], "", lines[2], lines[3], lines[4]]) + "\n")

    batches = list(
        ingest_jira_tickets._iter_ticket_batches_from_file(str(path), batch_size=2)
    )

    assert [len(b) for b in batches] == [2, 2, 1]
    flat = [ticket for batch in batches for ticket in batch]
    assert [ticket["key"] for ticket in flat] == [f"DEMO-{i}" for i in range(5)]
