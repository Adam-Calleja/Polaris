...
def test_run_shared_jira_fanout_index_fetches_once_processes_all_conditions_and_persists_once(
    ...
    first_batch_insert_indexes = [
        ...
        if event[0] == "insert" and event[1][1] == ["DEMO-1", "DEMO-2"]
    ]
    ...
    assert added_chunk_calls == [
        ("docstore:tickets_cs600_ov0", ["DEMO-1", "DEMO-2"]),
        ("docstore:tickets_cs800_ov0", ["DEMO-1", "DEMO-2"]),
        ("docstore:tickets_cs600_ov0", ["DEMO-3"]),
        ("docstore:tickets_cs800_ov0", ["DEMO-3"]),
    ]
    assert added_document_calls == [
        ("source-docstore:tickets_cs600_ov0", ["DEMO-1", "DEMO-2"]),
        ("source-docstore:tickets_cs800_ov0", ["DEMO-1", "DEMO-2"]),
        ("source-docstore:tickets_cs600_ov0", ["DEMO-3"]),
        ("source-docstore:tickets_cs800_ov0", ["DEMO-3"]),
    ]
...
def test_run_shared_jira_fanout_index_resumes_from_checkpoint_and_skips_completed_batches(
    ...
    assert inserted_batches == [["DEMO-2"]]
    assert result["summary"]["fetched_tickets"] == 3
    assert result["summary"]["processed_tickets"] == 3
    assert result["summary"]["processed_batches"] == 2
