...
function ProbeCard({ title, probe }: { title: string; probe: ApiProbeResult | null }) {
  if (!probe) {
    return (
      <div className="surface-card probe-card">
        <h3 className="probe-card__title">{title}</h3>
        <p className="probe-card__status">Loading...</p>
      </div>
    );
  }
    ...
    <section className="system-page">
      ...
      <section className="card-cluster">
        <div className="section-heading-row">
          ...
          <button className="secondary-button" onClick={() => setRefreshToken((value) => value + 1)} type="button">
            Refresh Status
          </button>
        </div>
        <div className="probe-grid">
          <ProbeCard probe={healthProbe} title="Health" />
          <ProbeCard probe={readyProbe} title="Readiness" />
        </div>
      </section>
      ...
      <section className="card-cluster">
        ...
        <div className="surface-card runtime-card">
          ...
          <div className="runtime-card__row">
            ...
            <code>{runtime?.feedback_log_path ?? (runtimeError ? runtimeError : "Loading...")}</code>
          </div>
        </div>
      </section>
    </section>
